...
import { Ticket, Video, Images, Newspaper, Snowflake, Instagram, Youtube, Facebook } from 'lucide-react';

// IMPORT COMPONENTS
import WeatherOverlay from './components/WeatherPage'; 
import PricesOverlay from './components/PricesPage'; 
import GalleryOverlay from './components/GalleryPage'; 
import NewsOverlay from './components/NewsPage'; 
import WebcamOverlay from './components/WebcamsPage'; 
...
const BanskoExperience: React.FC = () => {
  ...
  // --- ANIMATION VARIANTS ---
  // 1. Drop from Top (Home <-> Page)
  const dropVariants: Variants = {
    initial: { y: "-100%" },
    animate: { y: 0, transition: { duration: 0.8, ease: [0.16, 1, 0.3, 1] } },
    exit: { y: "-100%", transition: { duration: 0.6, ease: [0.16, 1, 0.3, 1] } }
  };

  // 2. Crossfade/Zoom (Page <-> Page)
  ...
  const currentVariants = isSwitching ? switchVariants : dropVariants;

  return (
    <>
      <AnimatePresence mode="popLayout">
        {activePage === 'Weather' && (
          <WeatherOverlay key="weather" onClose={closeOverlay} onNavigate={handleNavigation} variants={currentVariants} />
        )}
        {activePage === 'Prices' && (
          <PricesOverlay key="prices" onClose={closeOverlay} onNavigate={handleNavigation} variants={currentVariants} />
        )}
        {activePage === 'Gallery' && (
          <GalleryOverlay key="gallery" onClose={closeOverlay} onNavigate={handleNavigation} variants={currentVariants} />
        )}
        {activePage === 'News' && (
          <NewsOverlay key="news" onClose={closeOverlay} onNavigate={handleNavigation} variants={currentVariants} />
        )}
        {activePage === 'Webcams' && (
          <WebcamOverlay key="webcams" onClose={closeOverlay} onNavigate={handleNavigation} variants={currentVariants} />
        )}
      </AnimatePresence>
      ...
      >
          ...
          <header className="absolute top-0 left-0 z-50 w-full p-8 flex justify-center">
            ...
            <nav className="flex items-center gap-2 px-6 py-2 bg-white/10 backdrop-blur-2xl border border-white/30 rounded-full text-white shadow-2xl">
              <div className="flex gap-5 border-r border-white/20 pr-5">
                <button onClick={() => handleNavigation('Prices')} className="hover:text-blue-300 hover:scale-110 transition-all cursor-pointer" title="See Prices">
                  <Ticket size={20} />
                </button>
                <button onClick={() => handleNavigation('Webcams')} className="hover:text-blue-300 hover:scale-110 transition-all cursor-pointer" title="Webcams">
                  <Video size={20} /> 
                </button>
                <button onClick={() => handleNavigation('Gallery')} className="hover:text-blue-300 hover:scale-110 transition-all cursor-pointer" title="Gallery">
                  <Images size={20} /> 
                </button>
                <button onClick={() => handleNavigation('News')} className="hover:text-blue-300 hover:scale-110 transition-all cursor-pointer" title="News">
                  <Newspaper size={20} /> 
                </button>
              </div>
        ...
        <section ref={section2Ref} className="relative z-10 h-screen w-full flex items-center justify-center p-8 overflow-hidden snap-start snap-always">
          ...
          <motion.div className="grid grid-cols-6 grid-rows-3 gap-5 max-w-7xl w-full h-[85vh] relative z-10 text-[#f0f0f0]" initial={{ opacity: 0, scale: 0.98 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6 }}>
            {/* WEBCAMS */}
            <motion.div whileHover={popOut} onClick={() => handleNavigation('Webcams')} className="col-span-2 row-span-1 bg-white/20 backdrop-blur-xl border border-white/30 rounded-[2rem] p-3 flex justify-between items-center text-white relative overflow-hidden cursor-pointer">
              ...
              <div className="w-[50%] h-full rounded-3xl bg-cover bg-center shadow-inner" style={{backgroundImage: 'url("/pictures/webcams.png")'}} />
            </motion.div>

            {/* WEATHER */}
            <motion.div whileHover={popOut} onClick={() => handleNavigation('Weather')} className="col-span-2 row-span-1 bg-white/20 backdrop-blur-xl border border-white/30 rounded-[2rem] p-3 flex gap-4 items-center text-white cursor-pointer">
               <div className="w-[50%] h-full rounded-3xl bg-cover bg-center shadow-inner" style={{backgroundImage: 'url("/pictures/weather.png")'}} />
            ...
            {/* NEWS */}
            <motion.div whileHover={popOut} onClick={() => handleNavigation('News')} className="col-span-2 row-span-2 bg-white/20 backdrop-blur-xl border border-white/30 rounded-[2rem] p-5 text-white flex flex-col relative overflow-hidden cursor-pointer">
              <span className="text-lg font-bold uppercase tracking-widest mb-3">News</span>
              <div className="w-full h-[60%] rounded-2xl bg-cover bg-center mb-3 border border-white/10 shadow-lg" style={{backgroundImage: 'url("/pictures/news.png")'}} />
              <div className="flex flex-col items-center text-center mt-1">
                <p className="text-md font-bold leading-tight mb-4 px-2">Official start of winter season<br/>2025-2026 in Bansko</p>
                <button className="px-5 py-2 bg-white/20 hover:bg-white/30 transition-colors backdrop-blur-md rounded-xl text-[12px] border border-white/40 uppercase font-bold tracking-wider">More Information</button>
              </div>
            </motion.div>

            {/* SKI MAP - STATIC */}
            <div className="col-span-2 row-span-2 bg-white/10 backdrop-blur-xl border border-white/30 rounded-[2.5rem] relative overflow-hidden p-2">
              <div className="w-full h-full rounded-[2rem] bg-cover bg-center" style={{backgroundImage: 'url("/pictures/map.png")'}} />
            ...
            {/* GALLERY */}
            <motion.div whileHover={popOut} onClick={() => handleNavigation('Gallery')} className="col-span-2 row-span-1 bg-white/20 backdrop-blur-xl border border-white/30 rounded-[2rem] relative overflow-hidden cursor-pointer p-2">
              <div className="w-full h-full rounded-3xl bg-cover bg-center opacity-80 hover:opacity-100 transition-opacity" style={{backgroundImage: 'url("/pictures/gallery.png")'}} />
            ...
            <motion.div whileHover={popOut} onClick={() => handleNavigation('Prices')} className="col-span-2 row-span-1 bg-white/20 backdrop-blur-xl border border-white/30 rounded-[2rem] relative overflow-hidden cursor-pointer p-2">
              <div className="w-full h-full rounded-3xl bg-cover bg-center opacity-80" style={{backgroundImage: 'url("/pictures/prices.png")'}} />
        ...
        <section ref={section3Ref} className="relative z-5 h-screen w-full flex flex-col items-center justify-center px-6 pt-10 overflow-hidden snap-start snap-always">
          ...
          <div className="max-w-6xl w-full grid grid-cols-1 md:grid-cols-12 gap-8 mb-16 items-end relative z-20 md:ml-auto">
              <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="md:col-span-5 bg-white/10 backdrop-blur-2xl border border-white/20 rounded-[2.5rem] p-10 text-white shadow-2xl flex flex-col items-center">
                  <h3 className="text-xl font-bold mb-8 uppercase tracking-widest">Discover Bansko</h3>
                  <div className="grid grid-cols-2 gap-y-6 text-base w-full">
                      {['Webcams', 'Gallery', 'Weather', 'News', 'Ski Pass', 'Prices'].map(l => (
                        <div key={l} className="flex justify-center relative pb-4">
                          <button onClick={() => handleNavigation(l)} className="cursor-pointer hover:text-white/60 transition-colors uppercase text-sm tracking-wide font-medium">
                            {l}
                          </button>
                          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-3/4 border-b border-white/20"></div>
                        </div>
                      ))}
                  </div>
              </motion.div>
              <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className="md:col-span-4 bg-white/10 backdrop-blur-2xl border border-white/20 rounded-[2.5rem] p-11 text-white shadow-2xl flex flex-col items-center justify-center">
                  <h3 className="text-xl font-bold mb-8 uppercase tracking-widest">Awarded</h3>
                  <div className="grid grid-cols-2 gap-5 items-center">
                      <img src="/pictures/award1.png" alt="A1" className="h-25 w-25 object-contain" />
                      <img src="/pictures/award2.png" alt="A2" className="h-25 w-25 object-contain" />
                      <img src="/pictures/award3.png" alt="A3" className="h-25 w-25 object-contain" />
                      <img src="/pictures/award4.jpg" alt="A4" className="h-25 w-25 object-contain" />
                  </div>
              </motion.div>
          </div>
