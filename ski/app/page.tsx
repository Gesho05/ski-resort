...
const BanskoExperience: React.FC = () => {
  ...
  // Helper to handle footer clicks
  const handleFooterClick = (linkName: string) => {
    if (linkName === 'Webcams') setShowWebcams(true);
    if (linkName === 'Gallery') setShowGallery(true);
    if (linkName === 'Weather') setShowWeather(true);
    if (linkName === 'News') setShowNews(true);
    if (linkName === 'Prices') setShowPrices(true);
    // Add logic for others if/when you have components for them
  };
  ...
  return (
    <>
      <AnimatePresence>
        ...
        {showWebcams && <WebcamOverlay onClose={() => setShowWebcams(false)} />}
      </AnimatePresence>
            ...
            <nav className="flex items-center gap-2 px-6 py-2 bg-white/10 backdrop-blur-2xl border border-white/30 rounded-full text-white shadow-2xl">
              ...
              <div className="flex gap-4 pl-3 items-center text-[15px] font-semibold">
                ...
                <button 
                  ...
                  className="hover:text-blue-300 hover:scale-105 transition-all cursor-pointer"
                >
                  0.7° C
                </button>
        ...
        <section ref={section2Ref} className="relative z-10 h-screen w-full flex items-center justify-center p-8 overflow-hidden snap-start snap-always">
          ...
          >
            ...
            <motion.div 
              whileHover={popOut} 
              onClick={() => setShowWebcams(true)}
              className="col-span-2 row-span-1 bg-white/20 backdrop-blur-xl border border-white/30 rounded-[2rem] p-3 flex justify-between items-center text-white relative overflow-hidden cursor-pointer"
            >
            ...
            <motion.div whileHover={popOut} className="col-span-2 row-span-2 bg-white/10 backdrop-blur-xl border border-white/30 rounded-[2.5rem] relative overflow-hidden group cursor-pointer p-2">
              <div className="w-full h-full rounded-[2rem] bg-cover bg-center transition-transform duration-700 " style={{backgroundImage: 'url("/pictures/map.png")'}} />
                      ...
                      {['Webcams', 'Gallery', 'Weather', 'News', 'Ski Pass', 'Ski Map', 'Summer Offers', 'Prices'].map(l => (
                        <div key={l} className="flex justify-center relative pb-4">
                          <button 
                            ...
                            className="cursor-pointer hover:text-white/60 transition-colors uppercase text-sm tracking-wide font-medium"
                          >
                            {l}
                          </button>
