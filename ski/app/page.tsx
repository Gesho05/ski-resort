...
const BanskoExperience: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* --- SECTION 2: BENTO DASHBOARD (6x3 Layout with Updated Content) --- */}
      <section ref={section2Ref} className="relative z-10 h-screen w-full flex items-center justify-center p-8 overflow-hidden snap-start snap-always">
        ...
        <motion.div 
          // Reverted to grid-cols-6 grid-rows-3 but increased height
          className="grid grid-cols-6 grid-rows-3 gap-5 max-w-7xl w-full h-[85vh] relative z-10"
          initial={{ opacity: 0, scale: 0.98 }}
          whileInView={{ opacity: 1, scale: 1 }}
          transition={{ duration: 0.6 }}
        >
          {/* WEBCAMS (2x1) */}
          <motion.div whileHover={popOut} className="col-span-2 row-span-1 bg-white/20 backdrop-blur-xl border border-white/30 rounded-[2rem] p-5 flex justify-between items-center text-white relative overflow-hidden">
            <div className="z-10 flex flex-col justify-center h-full w-1/2">
              <div className="flex items-center gap-2 opacity-70 text-[11px] font-bold uppercase tracking-widest border-b border-white/30 pb-2 mb-2 w-fit">
                <Video size={16} /> Webcams
              </div>
              <p className="text-[10px] opacity-80 leading-snug">Live footage of all<br/>ski tracks in resort</p>
            </div>
            {/* Inner Box Image */}
            <div className="w-[100px] h-full rounded-2xl bg-cover bg-center border-2 border-white/20 shadow-inner" style={{backgroundImage: 'url("/pictures/image 2.jpg")'}} />
          </motion.div>

          {/* WEATHER (2x1) */}
          <motion.div whileHover={popOut} className="col-span-2 row-span-1 bg-white/20 backdrop-blur-xl border border-white/30 rounded-[2rem] p-3 flex gap-4 items-center text-white">
             {/* Left Image Box */}
             <div className="w-[50%] h-full rounded-2xl bg-cover bg-center shadow-inner" style={{backgroundImage: 'url("/pictures/photo2.png")'}} />
             <div className="flex flex-col justify-center w-[50%] h-full bg-white/10 rounded-2xl p-2">
               <span className="text-[10px] font-bold opacity-70 uppercase tracking-wider mb-1">Weather</span>
               <div className="flex flex-col items-center justify-center gap-0">
                 <span className="text-[9px] uppercase">Monday 12</span>
                 <Snowflake size={24} className="my-1" />
                 <span className="text-xl font-bold">-1° C</span>
               </div>
             </div>
          </motion.div>
          ...
          <motion.div whileHover={popOut} className="col-span-2 row-span-2 bg-white/20 backdrop-blur-xl border border-white/30 rounded-[2rem] p-5 text-white flex flex-col relative overflow-hidden">
            <span className="text-[11px] font-bold uppercase tracking-widest mb-3 opacity-90">News</span>
            {/* Inner News Image */}
          ...
          <motion.div whileHover={popOut} className="col-span-2 row-span-2 bg-white/10 backdrop-blur-xl border border-white/30 rounded-[2.5rem] relative overflow-hidden group cursor-pointer p-2">
            ...
            <div className="absolute bottom-6 right-8 text-[#135285] text-xl font-black italic tracking-tighter drop-shadow-sm uppercase bg-white/80 px-3 py-1 rounded-lg backdrop-blur-sm">Ski Map</div>
          </motion.div>
          ...
          <motion.div whileHover={popOut} className="col-span-2 row-span-1 bg-white/20 backdrop-blur-xl border border-white/30 rounded-[2rem] relative overflow-hidden cursor-pointer p-2">
            <div className="w-full h-full rounded-2xl bg-cover bg-center opacity-80 hover:opacity-100 transition-opacity" style={{backgroundImage: 'url("/pictures/photo2.png")'}} />
            <span className="absolute top-5 left-6 text-[#135285] text-[12px] font-black italic uppercase tracking-tighter bg-white/80 px-2 py-0.5 rounded-md">Gallery</span>
          </motion.div>
          ...
          <motion.div whileHover={popOut} className="col-span-2 row-span-1 bg-white/20 backdrop-blur-xl border border-white/30 rounded-[2rem] p-2 text-white flex gap-2 items-center justify-between">
            <div className="flex flex-col justify-center items-center w-[40%] h-full bg-white/10 rounded-2xl py-2">
                <span className="text-[9px] font-bold opacity-60 uppercase mb-1">Ski Pass</span>
            ...
            {/* Image on right side */}
            <div className="w-[60%] h-full rounded-2xl bg-cover bg-center border border-white/10" style={{backgroundImage: 'url("/pictures/photo1.png")'}} />
          </motion.div>

          {/* PRICES (2x1 Bottom Right) */}
          <motion.div whileHover={popOut} className="col-span-2 row-span-1 bg-white/20 backdrop-blur-xl border border-white/30 rounded-[2rem] relative overflow-hidden cursor-pointer p-2">
            <div className="w-full h-full rounded-2xl bg-cover bg-center opacity-80" style={{backgroundImage: 'url("/pictures/photo2.png")'}} />
            <span className="absolute top-6 left-6 text-[#135285] text-[12px] font-black italic uppercase tracking-tighter bg-white/80 px-2 py-0.5 rounded-md">Prices</span>
          </motion.div>
        </motion.div>
      </section>

      {/* --- SECTION 3: FOOTER --- */}
      <section ref={section3Ref} className="relative z-10 h-screen w-full flex flex-col items-center justify-center px-6 pt-10 overflow-hidden snap-start snap-always">
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden w-full h-full">
        ...
        <div className="max-w-7xl w-full grid grid-cols-1 md:grid-cols-12 gap-8 mb-16 items-end relative z-20">
